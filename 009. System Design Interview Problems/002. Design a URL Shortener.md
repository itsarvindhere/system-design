...
For example, a payment ID needs strong uniqueness. A timeline post ID may also benefit from time ordering. A random tracking ID may not need ordering.
...
One approach is to create a separate ID Generator Service. All the app servers call this service when they need a new ID.  The ID Generator Service increments a counter and returns a new ID.
...
```
    ...
    - CDN / Edge Cache can cache popular redirects, especially if links are permanent.
    - Load Balancer distributes traffic across stateless service instances.
    - Rate Limiter protects URL creation and redirects from abuse.
    - Distributed ID Generator creates unique IDs for short codes.
    - Distributed Cache stores short_code -> long_url for fast redirects.
    - Shard Router decides which database shard owns a short_code.
    - Database Sharding helps when one database cannot store or serve all mappings.
    - Replication improves read scalability and availability.
    - Message Queue keeps analytics out of the redirect path.
