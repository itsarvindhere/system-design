...
The WebSocket servers are multiple because they need to handle many long-lived connections.

When a user connects, the WebSocket server authenticates the user and stores connection mapping in Redis. For example -

    user_A -> WS1
    user_B -> WS3

    1. User A sends message to WS1
    2. WS1 sends message to Message Service
    3. Message Service stores message in Message DB
    4. Message Service publishes message event to Queue
    5. Delivery Worker consumes event
    6. Delivery Worker checks Redis for recipient connection
    7. Redis says User B is connected to WS3
    8. Delivery Worker sends message to WS3
    9. WS3 pushes message to User B
